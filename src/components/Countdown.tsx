  ...
  return (
    <section className="section bg-wedding wedding-corners relative overflow-hidden">
      <div className="container-page text-center space-y-10">
        {/* === Countdown Box — portrait 3:4, semua konten di dalam gambar === */}
        <div className="relative mx-auto w-full max-w-[420px] rounded-3xl overflow-hidden shadow-soft">
          {/* Portrait box wrapper */}
          <div className="relative aspect-[3/4]">
            {/* Zoom-out background image (portrait) */}
            <motion.div
              initial={{ scale: 1.08 }}
              animate={{ scale: 1.0 }}
            ...
            <div className="absolute inset-0 z-10 px-4 py-8 flex flex-col items-center justify-center text-white">
              ...
              <motion.div
                ...
                className="mb-4"
              >
                <p className="font-script text-[34px] leading-none text-[#B89B75] drop-shadow-[0_2px_8px_rgba(0,0,0,.45)] max-[360px]:text-[30px]">
                  Save The Date
                </p>
                <div className="h-px w-16 mx-auto my-2 bg-white/50" />
              </motion.div>
              ...
              <motion.div
                ...
                initial="hidden"
                animate="show"
                className="flex justify-center flex-nowrap gap-2 px-1 w-full"
                aria-live="polite"
                aria-atomic="true"
              >
                {boxes.map(([k, v]) => (
                  <motion.div
                    ...
                    variants={itemVar}
                    className="
                      flex-shrink-0 backdrop-blur-sm bg-white/10 border border-white/20
                  ...
                  >
                    <div
                      className="
                        display text-2xl font-semibold leading-none tracking-tight tabular-nums
                        text-[#B89B75] drop-shadow-[0_2px_4px_rgba(0,0,0,0.45)]
                        max-[360px]:text-xl
                      "
                    >
                      {k === "Days" ? v : pad2(v)}
                    </div>
                    <div className="text-[11px] mt-1 text-white/85 tracking-wide uppercase max-[360px]:text-[10px]">
                      {k}
                    </div>
                  </motion.div>
                ))}
        ...
        {/* === Token & Love Banner — tinggi dinaikkan (h-48) === */}
        <div className="relative mx-auto w-full max-w-[420px] rounded-2xl overflow-hidden shadow-soft">
